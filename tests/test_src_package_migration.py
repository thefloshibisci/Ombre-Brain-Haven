...
# Haven intentionally retains these root-level sidecars for Gateway/raw archive
# compatibility; they are not the upstream src package migration targets.
HAVEN_SIDECAR_MODULES = {"media_store", "backup_archive", "embedding_outbox"}
...
def test_flat_module_is_retired_and_canonical_module_imports(
    ...
    if legacy_name in HAVEN_SIDECAR_MODULES:
        assert spec is not None
        assert Path(spec.origin).resolve() == (ROOT / f"{legacy_name}.py").resolve()
    else:
        assert spec is None

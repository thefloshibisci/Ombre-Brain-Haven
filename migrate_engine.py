...
class MigrateEngine:
    ...
    async def _apply_one_bucket(
        ...
        *,
        ...
        turn_factory = getattr(self._bucket_mgr, "_derived_index_locks", None)
        if isinstance(turn_factory, dict):
            lock = turn_factory.get(pb.bucket_id)
            if lock is None:
                lock = asyncio.Lock()
                turn_factory[pb.bucket_id] = lock
            turn = lock
        else:
            turn = _noop_bucket_turn()
        async with turn:
